...
import { ShieldAlert, Share2, Globe, AlertTriangle, Scan, Fingerprint, Zap } from "lucide-react"
...
export function ScamAlertCard({ result, brandName }: { result: any, brandName: string }) {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-primary font-bold text-xs uppercase tracking-[0.3em]">Syndicate OS</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Digital Evidence ID</p>
              <p className="text-xl font-mono text-zinc-300 font-bold">#{Math.random().toString(36).substring(2, 10).toUpperCase()}</p>
            </div>
          </div>

          {/* Main Threat Manifest */}
          ...
          <div className="mt-auto flex justify-between items-end relative z-10 border-t border-white/5 pt-8">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Authenticity Guaranteed by AI</span>
              </div>
              <div className="flex items-center gap-2">
                <Globe className="text-emerald-500 h-5 w-5" />
                <span className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Community Protection Node</span>
              </div>
            </div>
            <div className="text-right">
              <p className="text-xl font-black uppercase italic text-white leading-none">Protect your peers.</p>
              <p className="text-primary font-bold text-sm tracking-widest">studentguard.pro</p>
            </div>
          </div>
      ...
      {/* Visual Button with Interaction */}
      <motion.button 
      ...
      <p className="text-[10px] text-center text-muted-foreground font-bold uppercase tracking-widest italic">
        The Dispatch Card is optimized for LinkedIn, Twitter, and University Discord channels.
      </p>
    </div>
  )
}
