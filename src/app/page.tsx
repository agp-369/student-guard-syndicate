        ...
        <div className="max-w-6xl mx-auto text-center space-y-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto font-medium italic opacity-70">
              "We are building the KYO (Know Your Offer) standard for the global student community. Forensic verification at the speed of thought."
            </p>
          </motion.div>
        </div>
      </header>

      {/* 🛡️ THE MISSION STORY (Truecaller-Style Trust Nodes) */}
      <section className="py-24 z-10 relative border-y border-border bg-card/10 backdrop-blur-2xl overflow-hidden">
      ...
      <section className="py-24 z-10">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="p-10 rounded-[3rem] bg-card border border-border shadow-2xl relative group overflow-hidden">
              <div className="flex justify-between items-center mb-10 pb-6 border-b border-border">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} className="h-12 w-12 rounded-2xl bg-accent border border-border flex items-center justify-center text-muted-foreground hover:text-primary transition-all group/up">
                    <FileUp size={20} className="group-hover/up:scale-110 transition-transform" />
                  </button>
              ...
              <div className="space-y-6 relative">
                ...
                <div className="relative">
                  ...
                  <AnimatePresence>
                    {isScanning && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-background/95 backdrop-blur-md rounded-3xl flex flex-col items-center justify-center p-12 border-2 border-primary/30 z-30">
                        ...
                        <div className="w-full max-w-xs space-y-4">
                          <p className="font-mono text-primary font-bold text-[10px] tracking-[0.5em] text-center">{SCAN_STEPS[scanStep]}</p>
              ...
              {result && (
                <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} className="mt-16 space-y-12">
                  ...
                  <div className={cn("p-12 rounded-[4rem] border-2 space-y-10 relative overflow-hidden shadow-inner", 
                    ...
                    <div className="flex justify-between items-end relative z-10">
                      <div className="space-y-3">
                        ...
                        <h4 className={cn("text-7xl font-black uppercase italic tracking-tighter leading-none", 
                    ...
                    <p className="text-2xl font-medium text-foreground italic leading-relaxed border-l-8 border-primary pl-10 py-4 opacity-90">"{result.analysis}"</p>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="p-10 rounded-[3rem] bg-card/60 backdrop-blur-3xl border border-border shadow-2xl relative overflow-hidden min-h-[700px] flex flex-col">
              <div className="flex items-center justify-between border-b border-border pb-8 mb-10 shrink-0">
                <div className="flex items-center gap-4">
                  <Network className="h-6 w-6 text-primary" />
                  <span className="text-sm font-black uppercase tracking-[0.4em]">Grid_Status</span>
                </div>
              ...
              <div className="flex-1 flex flex-col space-y-6">
                ...
                <div className="flex-1 relative bg-zinc-950/80 rounded-[2rem] border border-zinc-800 p-6 font-mono text-[10px] overflow-hidden">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="text-primary/40 animate-pulse uppercase tracking-[0.2em] mt-8 italic">Maintaining Core Handshake...</div>
                  </div>
                </div>
              </div>

              <div className="mt-10 p-6 rounded-3xl bg-primary text-background flex items-center gap-5 shadow-2xl shadow-primary/20 hover:scale-105 transition-all cursor-default group">
                <ShieldCheck size={32} className="group-hover:rotate-12 transition-transform" />
                <div>
                  <p className="text-[8px] font-black uppercase tracking-widest leading-none opacity-70">Syndicate_Status</p>
                  <p className="text-lg font-black uppercase italic leading-none mt-1">Verified Authority</p>
                </div>
              </div>
            </div>
          </div>
...
function VisualStoryNode({ icon, title, desc }: any) {
  return (
    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="space-y-6 group">
      <div className="h-24 w-24 rounded-[2rem] bg-accent border border-border flex items-center justify-center group-hover:scale-110 group-hover:shadow-[0_0_40px_rgba(99,102,241,0.15)] transition-all duration-500">
        {icon}
      </div>
      <div className="space-y-3">
...
function PulseMetric({ label, value, color = "text-primary" }: any) {
  return (
    <div className="p-6 rounded-[2rem] bg-background border border-border flex flex-col justify-center hover:border-primary/20 transition-all">
...
function InputTerminal({ icon, placeholder, value, onChange }: any) {
  return (
    <div className="relative group/in">
      ...
      <input className="w-full h-14 bg-background/80 border border-border rounded-2xl pl-12 pr-6 font-mono text-[10px] focus:border-primary outline-none text-foreground transition-all uppercase placeholder:text-muted-foreground/30" placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)} />
    </div>
  )
}
