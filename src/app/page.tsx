  ...
  const runScan = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(responseText || "Critical Node Error");
      }
        ...
        <div className="max-w-6xl mx-auto text-center z-10 relative">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-[10px] font-black uppercase tracking-[0.3em] mb-8">
              <Zap className="h-3 w-3 fill-primary" /> Community Defense Node v1.3
            </div>
            ...
            <p className="text-lg lg:text-xl text-muted-foreground mb-12 max-w-2xl mx-auto italic font-medium">
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="grid lg:grid-cols-12 gap-12">
            <div className="lg:col-span-7 space-y-8">
              <div className="p-10 rounded-[3rem] bg-card border border-border space-y-10 shadow-xl relative overflow-hidden">
                <div className="flex justify-between items-center relative z-10">
                  <h3 className="text-2xl font-black text-foreground uppercase italic tracking-tight">Forensic Probe</h3>
                ...
                <div className="flex gap-2 p-1 rounded-2xl bg-accent border border-border w-fit relative z-10">
                  <button onClick={() => setActiveProtocol("Offer")} className={cn("px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", activeProtocol === "Offer" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}>Full Offer</button>
                  <button onClick={() => setActiveProtocol("Domain")} className={cn("px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", activeProtocol === "Domain" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}>Domain</button>
                  <button onClick={() => setActiveProtocol("Email")} className={cn("px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", activeProtocol === "Email" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}>Email</button>
                </div>
                ...
                <div className="space-y-4 relative z-10">
                  ...
                  <textarea className="w-full h-48 bg-accent/50 border-2 border-border rounded-[2rem] p-6 text-sm font-medium focus:border-primary outline-none text-foreground" placeholder="Paste suspicious content here..." value={content} onChange={e => setContent(e.target.value)} />
                </div>

                <button onClick={runScan} disabled={isScanning || !content} className="w-full h-16 text-xl font-black rounded-3xl bg-primary text-primary-foreground hover:opacity-90 transition-all uppercase italic tracking-tighter flex items-center justify-center gap-3">
                  {isScanning ? <Loader2 className="animate-spin h-6 w-6" /> : <><FileSearch className="h-6 w-6" /> Run Security Scan</>}
                </button>
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="p-8 rounded-[2.5rem] bg-card border-2 border-border space-y-8 shadow-xl">
                <div className="flex items-center justify-between border-b border-border pb-4 mb-4">
                  <div className="flex items-center gap-3 text-primary"><Activity className="h-4 w-4" /><span className="text-xs font-black uppercase text-foreground tracking-widest">Community Pulse</span></div>
                ...
                <div className="space-y-4">
                  <h4 className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Recent Signatures</h4>
