...
import { AlertTriangle, ShieldCheck, Globe, Activity, Loader2, RefreshCw } from "lucide-react"
...
export default function IntelligenceHub() {
  ...
  useEffect(() => { 
    refreshLedger();
    const interval = setInterval(refreshLedger, 30000); // Background refresh
  ...
  const refreshLedger = async () => {
    ...
    if (data) {
      if (view === "scams") {
        setThreats(data.filter((t: any) => t.verdict === "SCAM"));
      } else if (view === "clear") {
        setThreats(data.filter((t: any) => t.verdict === "CLEAR"));
      } else {
        setThreats(data);
      }
    }
      ...
      <div className="grid gap-4">
        {isLoading && threats.length === 0 ? (
          <div className="py-32 flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-muted-foreground animate-pulse">Establishing_Ledger_Sync...</p>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={t.id} className="p-6 md:p-8 rounded-[2.5rem] bg-card/50 backdrop-blur-3xl border border-border flex flex-col md:flex-row items-center justify-between gap-8 group relative overflow-hidden">
            ...
            <div className="flex items-center gap-8 w-full text-left">
              <div className={`h-16 w-16 rounded-2xl flex items-center justify-center border shadow-2xl ${t.verdict === 'SCAM' ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'}`}>
                {t.verdict === 'SCAM' ? <AlertTriangle size={28} /> : <ShieldCheck size={28} />}
              </div>
              <div className="space-y-1">
                <h4 className="text-2xl font-black uppercase italic text-foreground tracking-tight">{t.brand_name}</h4>
                <p className="text-[10px] font-mono text-muted-foreground mt-1 uppercase tracking-widest leading-none">{t.domain} // {t.category}</p>
              </div>
            </div>
            <div className="flex items-center gap-10 shrink-0 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 md:border-l border-border pt-6 md:pt-0 md:pl-10">
              <div className="text-right">
                <p className="text-[8px] font-black text-muted-foreground uppercase tracking-widest opacity-60">Status</p>
                <p className={`text-xs font-bold mt-1 ${t.verdict === 'SCAM' ? 'text-red-500' : 'text-emerald-500'}`}>{t.verdict === 'SCAM' ? 'NEUTRALIZED' : 'VERIFIED_CLEAR'}</p>
              </div>
              <div className="text-right">
                <p className="text-[8px] font-black text-muted-foreground uppercase tracking-widest opacity-60">Timestamp</p>
                <p className="text-xs font-mono font-bold text-foreground mt-1">{new Date(t.created_at).toLocaleDateString()}</p>
              </div>
            </div>
          </motion.div>
        )) : (
          <div className="py-48 text-center border-2 border-dashed border-border rounded-[3rem] opacity-20 italic font-black text-4xl uppercase tracking-tighter leading-none">
            Ledger_Empty
          </div>
        )}
      </div>
